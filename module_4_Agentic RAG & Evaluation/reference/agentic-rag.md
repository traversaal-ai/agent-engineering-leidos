...
The module includes a live, hands-on comparison of a Naive RAG pipeline against an Agentic RAG pipeline side by side, this is where the four ingredients above (routing, one-shot query planning, tool use, conversation memory) are seen actually changing the pipeline's behavior on the same queries that broke naive RAG in Module 3.

**That comparison is [`axis/`](../axis/), in this module's folder.** It holds the two strategies to one variable: same documents, same index, same embedding model, same synthesis prompt, so any difference in cost, latency or answer quality is attributable to the orchestration and nothing else. Three of the four ingredients above appear in it directly — the Router, the Decomposer (one-shot query planning), and conversation memory read by a query rewriter — plus tool use in the form of a document search tool and an optional web search tool inside a bounded ReAct loop. Setup and the questions to run are in the module [`README.md`](../README.md).

Note where it sits on the spectrum drawn above. Axis's loop is a **bounded** ReAct, entered only where the cheap path failed or where the router classified the question as multi-hop — which makes it a deliberate midpoint between the ingredients column and the full-agents column, rather than either one. The bound is the interesting part: an unbounded loop on a teaching budget is a way to spend a class's money discovering nothing.
